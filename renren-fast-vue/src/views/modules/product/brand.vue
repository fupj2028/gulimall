      ...
      <el-table-column prop="logo" header-align="center" align="center" label="品牌logo地址">
        <template slot-scope="scope">
          <!-- <el-image
              ...
              :src="scope.row.logo"
          fit="fill"></el-image>-->
          <el-image :src="logoUrls[scope.row.brandId]" style="width: 100px; height: 80px" fit="fill"></el-image>
        </template>
      </el-table-column>
      ...
      this.dataList.forEach(item => {
        if (item.logo) {
          this.$http({
            ...
            method: 'get',
            params: this.$http.adornParams({key: item.logo})
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$set(this.logoUrls, item.brandId, data.data)
            }
          })
        }
      })
