  ...
  >
    ...
    >
      <el-form-item label="采购商品" prop="skuId">
        <el-select v-model="dataForm.skuId" filterable remote placeholder="请输入关键字搜索商品" :remote-method="searchSkus" :loading="skuSearchLoading" style="width:100%">
          <el-option v-for="item in skuList" :key="item.skuId" :label="item.skuName" :value="item.skuId">
            ...
            <span style="float:right;color:#8492a6;font-size:12px">ID: {{ item.skuId }}</span>
          </el-option>
        </el-select>
      </el-form-item>
...
export default {
  ...
  methods: {
    searchSkus(query) {
      if (!query) return;
      this.skuSearchLoading = true;
      this.$http({
        url: this.$http.adornUrl("/product/skuinfo/list"),
        method: "get",
        params: this.$http.adornParams({ key: query, limit: 20 })
      }).then(({ data }) => {
        this.skuList = data.page.list;
        this.skuSearchLoading = false;
      }).catch(() => {
        this.skuSearchLoading = false;
      });
    },
        ...
        if (this.dataForm.id) {
          this.$http({
            ...
            if (data && data.code === 0) {
              ...
              this.$http({
                url: this.$http.adornUrl(`/product/skuinfo/info/${data.purchaseDetail.skuId}`),
                method: "get",
                params: this.$http.adornParams()
              }).then(({ data: skuResp }) => {
                if (skuResp && skuResp.code === 0) {
                  this.skuList = [{ skuId: skuResp.skuInfo.skuId, skuName: skuResp.skuInfo.skuName }];
                }
              });
